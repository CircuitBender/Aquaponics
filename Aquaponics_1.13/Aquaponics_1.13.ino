...
const unsigned long FlowbackTime = 76000L;
...
const unsigned long SensorCycle = 10005;
...
/*const int afterWateringEvent = t.after((FlowbackTime)*1L, xyz);
...
void loop(){
   ...
   if ((unsigned long)(currentMillis1 - previousMillisUpdate1) >= intervallUpdate) {
    ...
    digitalClockDisplay();
    t.update();      // update every 1 second
    previousMillisUpdate1 = currentMillis1;
   }
   if ((unsigned long)(currentMillis2 - previousMillisUpdate2) >= SensorCycle) {
    digitalClockDisplay();
    readSensors();
    t.update();      // read sensors every 10 sec
    previousMillisUpdate2 = currentMillis2;
   }
 if ((unsigned long)(currentMillis2 - previousMillisUpdate2) >= CycleTime) {
    digitalClockDisplay();
    abc();
    t.update();      // read sensors every 10 sec
    previousMillisUpdate2 = currentMillis2;
   }
 if ((unsigned long)(currentMillis2 - previousMillisUpdate2) >= NTPupdateTime) {
    digitalClockDisplay();
    getNtptime();
    t.update();      // read sensors every 10 sec
    previousMillisUpdate2 = currentMillis2;
...
do {
   ...
   if ((unsigned long)(currentMillisWatering - previousMillisWatering) >= WateringTime) {
