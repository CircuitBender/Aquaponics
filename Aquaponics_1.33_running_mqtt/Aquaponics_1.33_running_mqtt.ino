...
#define ECHOPIN 43        // Pin to receive echo pulse 
...
void dht11() {
  ...
  */
  ...
  if (isnan(event1.temperature)) {
    ...
    float dht11temperature = event1.temperature;
  ...
  if (isnan(event1.relative_humidity)) {
    ...
    float dht11humidity = event1.relative_humidity;
  ...
  if (isnan(event2.temperature)) {
    ...
    Serial.println(" °C Air Temp (DHT11)");
    float dht11temperature = event2.temperature;
    }
  // Get humidity event and print its value.
