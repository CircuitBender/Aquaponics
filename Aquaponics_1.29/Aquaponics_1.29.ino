...
const int LightsMorningONhour = 10; const int LightsMorningONminute = 07; const int LightsMorningONsec = 3;
const int LightsMorningOFFhour = 13; const int LightsMorningOFFminute = 30; const int LightsMorningOFFsec = 3;
const int LightsEveningONhour = 14; const int LightsEveningONminute = 45; const int LightsEveningONsec = 3;
const int LightsEveningOFFhour = 23; const int LightsEveningOFFminute = 00; const int LightsEveningOFFsec = 3;

// ++++++++++++++++++ CYCLE TIMES ++++++++++++++++++++
const unsigned long WateringTime = 22023L;
const unsigned long FlowbackTime = 76019L;
const unsigned long CycleTime = (5*60*1000L);
const unsigned long CycleTimeHigh = (110*1000L);
const unsigned long NTPupdateTime = (4*60*1000L);
...
void setup () {
  ...
  Alarm.delay(11);
  Serial.println(" ");
  Serial.println("setting all RELAYS");
  digitalWrite(PumpeA_Relais1, HIGH);
  client.publish("Aquaponics/Relays/Out/PumpeA_Relais1", "1");
  ...
  Alarm.delay(11);
  Serial.println("setting PumpeK_Relais2: LOW");
  client.publish("Aquaponics/Relays/Out/PumpeK_Relais2", false);
  
  digitalWrite(LichtA_Relais3, LOW);
  Alarm.delay(11);
  Serial.println("setting LichtA_Relais3: LOW");
  client.publish("Aquaponics/Relays/Out/LichtA_Relais3", "0");
  
  digitalWrite(HeizstabA_Relais4, HIGH);
  Serial.println("setting HeizstabA_Relais4: HIGH");
  client.publish("Aquaponics/Relays/Out/HeizstabA_Relais4", "HIGH");
  Serial.println("");
  ...
  switch (TimeState) {
    case 1:
      ...
      Serial.println("setting PumpeA_Relais1: HIGH");
         char buffer[10];
  dtostrf(1,0, 0, buffer);
  client.publish("Aquaponics/Relays/Out/PumpeA_Relais1",buffer);
      Alarm.delay(11);
      digitalWrite(PumpeK_Relais2, LOW);
      Serial.println("setting PumpeK_Relais2: LOW");
       dtostrf(false,0, 0, buffer);
  client.publish("Aquaponics/Relays/Out/PumpeK_Relais2",buffer);
      Alarm.delay(11);
      digitalWrite(LichtA_Relais3, HIGH);
      Serial.println("setting LichtA_Relais3: HIGH");
       dtostrf(1,0, 0, buffer);
  client.publish("Aquaponics/Relays/Out/LichtA_Relais3",buffer);
      Alarm.delay(11);
      digitalWrite(HeizstabA_Relais4, HIGH);
      Serial.println("setting HeizstabA_Relais4: HIGH");
       dtostrf(1,0, 0, buffer);
  client.publish("Aquaponics/Relays/Out/HeizstabA_Relais4",buffer);
      Serial.println(" ");
      Alarm.delay(1000);        // delay in between reads for stability
      t1.enable();
      r.addTask(t1);
      dtostrf(true,0, 0, buffer);
  client.publish("Aquaponics/Tasks/Out/Task1",buffer);
      t2.enable();
      r.addTask(t2);
      
      dtostrf(1,0, 0, buffer);
  client.publish("Aquaponics/Tasks/Out/Task2",buffer);
      t3.enable();
      r.addTask(t3);
      
      dtostrf(1,0, 0, buffer);
  client.publish("Aquaponics/Tasks/Out/Task3",buffer);

      t4.enable();
      r.addTask(t4);
      
      dtostrf(1,0, 0, buffer);
  client.publish("Aquaponics/Tasks/Out/Task4",buffer);
      t4.setInterval(CycleTime);
      
      dtostrf(1,0, 0, buffer);
  client.publish("Aquaponics/Tasks/Out/Task4/Cycletime",buffer);
      Serial.println("WATERING IS ON");
...
void UltraSonic() {
  ...
  client.publish("Aquaponics/Sensors/WaterLevelAlarm", "ALARM WATERLEVEL!");
    ...
    Serial.print(WaterLevel);
     char buffer[10];
  dtostrf(WaterLevel,0, 0, buffer);
  client.publish("Aquaponics/Sensors/WaterLevel",buffer);
    Serial.print(" cm Waterlevel (29cm MAX) // ");
    
    Serial.print(distance);
    Serial.println(" cm von Decke (9 cm MAX)");
  //  char buffer[11];
  dtostrf(distance,0, 0, buffer);
  client.publish("Aquaponics/Sensors/distance",buffer);
  
    Serial.print(WaterLitres);
    Serial.print(" l Liter Wasser (");

   //     char buffer[12];
  dtostrf(WaterLitres,0, 0, buffer);
  client.publish("Aquaponics/Sensors/WaterLitres",buffer);
  
    Serial.print(WaterVolume);
    Serial.println(" cm^2 (60900 cm^2 MAX)");
     //   char buffer[13];
  dtostrf(WaterVolume,0, 0, buffer);
  client.publish("Aquaponics/Sensors/WaterVolume",buffer);
  }
}
...
void dht11() {
  ...
  if (isnan(event.temperature)) {
    ...
    float DHT11Temp = event.temperature;
   char buffer[10];
  dtostrf(DHT11Temp,0, 0, buffer);
  client.publish("Aquaponics/Sensors/DHT11_Temp",buffer);
  }
  ...
  if (isnan(event.relative_humidity)) {
    ...
    Serial.println(" %  Humidity (DHT11)");
     float DHT11Hum = event.relative_humidity;
   char buffer[10];
  dtostrf(DHT11Hum,0, 0, buffer);
  client.publish("Aquaponics/Sensors/DHT11_Hum",buffer);
  }
}
...
void GetWaterTEMP() {
  ...
  WaterTemperature = TX1 - 21.5; // Calibration
  Serial.print(WaterTemperature);
  Serial.println(" °C (Thermistor Water Temperature) ");
  
   char buffer[10];
  dtostrf(WaterTemperature,0, 0, buffer);
  client.publish("Aquaponics/Sensors/Thermistor_Water",buffer);
...
void GetAirTEMP() {
 ...
 char buffer[10];
  dtostrf(AirTemperature,0, 0, buffer);
  client.publish("Aquaponics/Sensors/Thermistor_Air",buffer);
...
void PrintECReadings() {
 ...
 char buffer[10];
  dtostrf(Rc,0, 0, buffer);
  client.publish("Aquaponics/Sensors/EC1_RC",buffer);
//   char buffer[11];
  dtostrf(EC25,0, 0, buffer);
  client.publish("Aquaponics/Sensors/EC1_EC25",buffer);
  // char buffer[12];
