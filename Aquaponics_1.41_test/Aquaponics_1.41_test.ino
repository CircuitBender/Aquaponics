/* <<<<<<<<<<<< Aquaponics 1.40 by Marco Heinzen >>>>>>>>>>>>>>>>>>>
      _ _
     _{ ' }_
    { `.!.` }
    ',_/Y\_,'
...
/*
  // Grove High Temp Needs to be fixed I2C collision
...
String dht11_1_temp_str; //see last code block below use these to convert the float that you get back from DHT to a string =str
String dht11_1_hum_str;
char dht11_1_temp_chr[50];
char dht11_1_hum_chr[50];

// mqtt Variables for printing temp and humidity
String dht11_2_temp_str; //see last code block below use these to convert the float that you get back from DHT to a string =str
String dht11_2_hum_str;
char dht11_2_temp_chr[50];
char dht11_2_hum_chr[50];


//  User Defined Variables
uint32_t DailySetupHour = 0 * 3600000L; uint32_t DailySetupMinute = 0 * 60000L; uint32_t DailySetupSec = 0 * 1000L;
uint32_t AquaSunriseHour = 0 * 3600000L; uint32_t AquaSunriseMinute = 0 * 60000L; uint32_t AquaSunriseSec = 0 * 1000L;
uint32_t AuqaMorningHour = 0 * 3600000L; uint32_t AuqaMorningMinute = 0 * 60000L; uint32_t AuqaMorningSec = 0 * 1000L;
uint32_t AquaAfternoonHour = 0 * 3600000L; uint32_t AquaAfternoonMinute = 0 * 60000L; uint32_t AquaAfternoonSec = 0 * 1000L;
uint32_t AquaEveningHour = 0 * 3600000L; uint32_t AquaEveningMinute = 0 * 60000L; uint32_t AquaEveningSec = 0 * 1000L;
uint32_t AquaNightHour = 0 * 3600000L; uint32_t AquaNightMinute = 0 * 60000L; uint32_t AquaNightSec = 0 * 1000L;
uint32_t GrowMorningHour = 0 * 3600000L; uint32_t GrowMorningMinute = 0 * 60000L; uint32_t GrowMorningSec = 0 * 1000L;
uint32_t GrowAfternoonHour = 0 * 3600000L; uint32_t GrowAfternoonMinute = 0 * 60000L; uint32_t GrowAfternoonSec = 0 * 1000L;
uint32_t GrowEveningHour = 0 * 3600000L; uint32_t GrowEveningMinute = 0 * 60000L; uint32_t GrowEveningSec = 0 * 1000L;
uint32_t GrowNightHour = 0 * 3600000L; uint32_t GrowNightMinute = 0 * 60000L; uint32_t GrowNightSec = 0 * 1000L;

uint32_t DailySetupMillis = (DailySetupHour + DailySetupMinute + DailySetupSec)*1L;
uint32_t AquaSunriseMillis = (AquaSunriseHour + AquaSunriseMinute + AquaSunriseSec)*1L;
uint32_t AuqaMorningMillis = (AuqaMorningHour + AuqaMorningMinute + AuqaMorningSec)*1L;
uint32_t AquaAfternoonMillis = (AquaAfternoonHour + AquaAfternoonMinute + AquaAfternoonSec)*1L;
uint32_t AquaEveningMillis = (AquaEveningHour + AquaEveningMinute + AquaEveningSec)*1L;
uint32_t AquaNightMillis = (AquaNightHour + AquaNightMinute + AquaNightSec)*1L;
uint32_t GrowMorningMillis = (GrowMorningHour + GrowMorningMinute + GrowMorningSec)*1L;
uint32_t GrowAfternoonMillis = (GrowAfternoonHour + GrowAfternoonMinute + GrowAfternoonSec)*1L;
uint32_t GrowEveningMillis = (GrowEveningHour + GrowEveningMinute + GrowEveningSec)*1L;
uint32_t GrowNightMillis = (GrowNightHour + GrowNightMinute + GrowNightSec)*1L;
...
//   ** Water Sensor  **
String waterdetection1_str; //see last code block below use these to convert the float that you get back from DHT to a string =str
String waterdetection2_str;
char waterdetection1_chr[50];
char waterdetection2_chr[50];
...
// ++++++++++++++++ Time +++++++++++++++++++++
// NTP Servers:
IPAddress timeServer1(132, 163, 4, 101); // time-a.timefreq.bldrdoc.gov
IPAddress timeServer2(132, 163, 4, 102); // time-b.timefreq.bldrdoc.gov
...
// mqtt Variables for printing temp
String therm_water_temp_str; //see last code block below use these to convert the float that you get back from DHT to a string =str
String therm_air_temp_str;
char therm_water_temp_chr[50];
char therm_air_temp_chr[50];
//##################################################################################
...
char EC2_EC2_chr[50];
String EC1_ppm1_str;
char EC1_ppm1_chr[50];
String EC2_ppm2_str;
char EC2_ppm2_chr[50];
...
/*
  ...
  do {
    ...
    unsigned long currentMillisafterSunrise = millis();
    if ((unsigned long)(currentMillisafterSunrise - previousMillisafterSunrise) >= Sunrise) {
      e++;
      previousMillisafterSunrise = currentMillisafterSunrise;
    }
  }
  while ((e * 10) < ((Sunrise) / 100));
  */
...
/*
  ...
  do {
    ...
    unsigned long currentMillisafterSunset = millis();
    if ((unsigned long)(currentMillisafterSunset - previousMillisafterSunset) >= Sunset) {

      q++;
      previousMillisafterSunset = currentMillisafterSunset;
    }
  }
  while ((q * 10) < (Sunset / 100L) );
  */
  digitalWrite(LichtA2_Relais3, LOW);
  Serial.println("setting LichtA2_Relais3: LOW");
...
void abc () {
  ...
  Serial.println(" Seconds to activate the next Cycle"); // Time in s
...
void fishfeeder() {
  ...
  if (f % 3 == 0) {
    ...
    while (w * 100 < ((FeedingQuantity) / 10));
    ...
    */
    ...
    Alarm.delay(15);
    client.publish("Aquaponics/Fishfeeder", "Fish fed");

  }
  else {
    Serial.println("Fishfeeder Skipped");
    client.publish("Aquaponics/Fishfeeder", "Fishfeeder Skipped");
  }
  feeder++;
}
...
void dht11() {
  ...
  if (isnan(event1.temperature)) {
    ...
    float dht11temperature1 = event1.temperature;
    dht11_1_temp_str = String(dht11temperature1); //converting ftemp (the float variable above) to a string
    dht11_1_temp_str.toCharArray(dht11_1_temp_chr, dht11_1_temp_str.length() + 1); //packaging up the data to publish to mqtt whoa...
    client.publish("Aquaponics/Sensors/1DHT11_Temp", dht11_1_temp_chr);
  ...
  if (isnan(event1.relative_humidity)) {
    ...
    float dht11humidity1 = event1.relative_humidity;
    dht11_1_hum_str = String(dht11humidity1); //converting Humidity (the float variable above) to a string
    dht11_1_hum_str.toCharArray(dht11_1_hum_chr, dht11_1_hum_str.length() + 1); //packaging up the data to publish to mqtt whoa...
    client.publish("Aquaponics/Sensors/1DHT11_Hum", dht11_1_hum_chr);
  ...
  if (isnan(event2.temperature)) {
    ...
    float dht11temperature2 = event2.temperature;
    dht11_2_temp_str = String(dht11temperature2); //converting ftemp (the float variable above) to a string
    dht11_2_temp_str.toCharArray(dht11_2_temp_chr, dht11_2_temp_str.length() + 1); //packaging up the data to publish to mqtt whoa...
    client.publish("Aquaponics/Sensors/2DHT11_Temp", dht11_2_temp_chr);
  ...
  if (isnan(event2.relative_humidity)) {
    ...
    float dht11humidity2 = event2.relative_humidity;
    dht11_2_hum_str = String(dht11humidity2); //converting Humidity (the float variable above) to a string
    dht11_2_hum_str.toCharArray(dht11_2_hum_chr, dht11_2_hum_str.length() + 1); //packaging up the data to publish to mqtt whoa...
    client.publish("Aquaponics/Sensors/2DHT11_Hum", dht11_2_hum_chr);
...
void GetWaterTEMP() {
  ...
  float Therm_WaterTemperature = TX1 - 0; // Calibration
  uint32_t Th_Water_Temp = Therm_WaterTemperature;
  Serial.print(Therm_WaterTemperature);
  Serial.println(" °C (Thermistor Water Temperature) ");

  therm_water_temp_str = String(Therm_WaterTemperature); //converting ftemp (the float variable above) to a string
  therm_water_temp_str.toCharArray(therm_water_temp_chr, therm_water_temp_str.length() + 1); //packaging up the data to publish to mqtt whoa...
  client.publish("Aquaponics/Sensors/Thermistor_Water", therm_water_temp_chr);
...
void GetAirTEMP() {
  ...
  float  Therm_AirTemperature = TX2 - 0;
  uint32_t Th_Air_Temp = Therm_AirTemperature;
  Serial.print(Therm_AirTemperature);
  Serial.println(" °C (Thermistor Air Temperature)");

  therm_air_temp_str = String(Therm_AirTemperature); //converting ftemp (the float variable above) to a string
  therm_air_temp_str.toCharArray(therm_air_temp_chr, therm_air_temp_str.length() + 1); //packaging up the data to publish to mqtt whoa...
  client.publish("Aquaponics/Sensors/Thermistor_Air", therm_air_temp_chr);
...
void Water_Detection () {
  ...
  Serial.println(" (0-1024)");
  waterdetection1_str = String(water_detection1); //converting ftemp (the float variable above) to a string
  waterdetection1_str.toCharArray(waterdetection1_chr, waterdetection1_str.length() + 1); //packaging up the data to publish to mqtt whoa...
  client.publish("Aquaponics/Sensors/WaterDetection/Floor/Value", waterdetection1_chr);
  waterdetection2_str = String(water_detection2); //converting ftemp (the float variable above) to a string
  waterdetection2_str.toCharArray(waterdetection2_chr, waterdetection2_str.length() + 1); //packaging up the data to publish to mqtt whoa...
  client.publish("Aquaponics/Sensors/WaterDetection/Grow/Value", waterdetection2_chr);

  if (water_detection1 > 250) {
    digitalWrite(PumpeA_Relais1, LOW);
    digitalWrite(PumpeK_Relais4, LOW);
    Serial.println(water_detection1);
    Serial.println("ALARM ! WASSER ! GROW !");

    client.publish("Aquaponics/Sensors/WaterDetection/Grow", "ALARM TOO MUCH WATER IN GROWBED DETECTED");


  }
  if (water_detection2 > 150) {
    digitalWrite(PumpeA_Relais1, LOW);
    digitalWrite(PumpeK_Relais4, LOW);
    Serial.println(water_detection1);
    Serial.println("ALARM ! WASSER ! BODEN !");
    client.publish("Aquaponics/Sensors/WaterDetection/Floor", "ALARM WATER ON FLOOR DETECTED");
...
void GetEC() {
  ...
  EC251  =  EC1 / (1 + TemperatureCoef * (Th_Water_Temp - 25.0));
  ...
  // Compensating For Temperaure
  EC252  =  EC2 / (1 + TemperatureCoef * (Th_Water_Temp - 25.0));
  ppm2 = (EC252) * (PPMconversion * 1000);

  EC1_Rc1_str = String(Rc1); //converting ftemp (the float variable above) to a string
  EC1_Rc1_str.toCharArray(EC1_Rc1_chr, EC1_Rc1_str.length() + 1); //packaging up the data to publish to mqtt whoa...
  client.publish("Aquaponics/Sensors/EC1_Rc1", EC1_Rc1_chr);

  EC2_Rc2_str = String(Rc2); //converting ftemp (the float variable above) to a string
  EC2_Rc2_str.toCharArray(EC2_Rc2_chr, EC2_Rc2_str.length() + 1); //packaging up the data to publish to mqtt whoa...
  client.publish("Aquaponics/Sensors/EC2_Rc2", EC2_Rc2_chr);

  EC1_EC1_str = String(EC251); //converting ftemp (the float variable above) to a string
  EC1_EC1_str.toCharArray(EC1_EC1_chr, EC1_EC1_str.length() + 1); //packaging up the data to publish to mqtt whoa...
  client.publish("Aquaponics/Sensors/EC1_EC1", EC1_EC1_chr);

  EC2_EC2_str = String(EC252); //converting ftemp (the float variable above) to a string
  EC2_EC2_str.toCharArray(EC2_EC2_chr, EC2_EC2_str.length() + 1); //packaging up the data to publish to mqtt whoa...
  client.publish("Aquaponics/Sensors/EC2_EC2", EC2_EC2_chr);

  EC1_ppm1_str = String(ppm1); //converting ftemp (the float variable above) to a string
  EC1_ppm1_str.toCharArray(EC1_ppm1_chr, EC1_ppm1_str.length() + 1); //packaging up the data to publish to mqtt whoa...
  client.publish("Aquaponics/Sensors/EC1_ppm1", EC1_ppm1_chr);

  EC2_ppm2_str = String(ppm2); //converting ftemp (the float variable above) to a string
  EC2_ppm2_str.toCharArray(EC2_ppm2_chr, EC2_ppm2_str.length() + 1); //packaging up the data to publish to mqtt whoa...
  client.publish("Aquaponics/Sensors/EC2_ppm2", EC2_ppm2_chr);
